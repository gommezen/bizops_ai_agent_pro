...
from agent.tools import needs as needs_mod
from agent.tools import profiling as dq_mod
from agent.tools import modeling as mdl_mod
from agent.tools.reporting import build_context, render_html, html_to_pdf
...
uploaded_texts = st.sidebar.file_uploader("Interview-filer (.txt)", accept_multiple_files=True, type=["txt"])
...
with tabs[2]:
    ...
    target = st.selectbox("Vælg target-kolonne", options=df.columns.tolist(), index=df.columns.tolist().index(target_default))
    shap_on = st.checkbox("Gem SHAP-plot (hvis klassifikation)", value=cfg.get("model",{}).get("shap", True))
    if st.button("Train baseline"):
        try:
            from pathlib import Path
            run_dir = Path("data/runs") / "manual_run"
            run_dir.mkdir(parents=True, exist_ok=True)
            model_info = mdl_mod.run_model(csv_path, target=target, test_size=cfg.get("model",{}).get("test_size",0.2), random_state=cfg.get("model",{}).get("random_state",42), want_shap=shap_on, run_dir=run_dir)
...
with tabs[3]:
    ...
    needs = needs_mod.run_needs(str(interviews_dir), clusters=cfg.get("needs",{}).get("clusters",4), top_terms=cfg.get("needs",{}).get("top_terms",8))
    dq = dq_mod.run_dq(csv_path)
    mdl = mdl_mod.run_model(csv_path, target=cfg.get("model",{}).get("target","churn"), test_size=cfg.get("model",{}).get("test_size",0.2), random_state=cfg.get("model",{}).get("random_state",42), want_shap=cfg.get("model",{}).get("shap", True), run_dir=Path("data/runs")/"manual_report")
    ...
    with open("data/runs/manual_report/executive_report.html","r",encoding="utf-8") as f:
        st.download_button("Download HTML report", data=f.read(), file_name="executive_report.html", mime="text/html")
    if st.button("Also export PDF"):
        ...
        with open(pdf_path, "rb") as f:
            st.download_button("Download PDF report", data=f.read(), file_name="executive_report.pdf", mime="application/pdf")
...
with tabs[4]:
    ...
    selected = st.multiselect("Vælg steps i rækkefølge", options=step_options, default=default_steps)
    ...
    if st.button("Run agent plan"):
        try:
            ...
            if "html_path" in result:
                ...
                if p.exists():
                    st.info(f"HTML: {p}")
                    st.download_button("Download HTML report", data=p.read_text(encoding="utf-8"), file_name="executive_report.html", mime="text/html")
            if "pdf_path" in result:
                from pathlib import Path
                p = Path(result["pdf_path"])
                if p.exists():
                    st.info(f"PDF: {p}")
                    st.download_button("Download PDF report", data=p.read_bytes(), file_name="executive_report.pdf", mime="application/pdf")
        except Exception as e:
            st.error(str(e))
